...
test.describe.only('Verify that the user is able to add the product into cart', () => {
  ...
  test.only('User add the product into add to cart page', { tag: ['@only'] }, async ({ Given, homepage, When, And, Then, walletpage, product_pages, addtocartpage }) => { 
    ...
    await And('the product added to cart properly', null, { product_pages, addtocartpage }); 
  });
